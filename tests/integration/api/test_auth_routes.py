...
async def test_login_redirect(client: AsyncClient):
    ...
    """
    response = await client.get("/api/v1/auth/google/login", params={"slack_user_id": "U12345"})
...
async def test_auth_callback(client: AsyncClient):
    ...
    response = await client.get("/api/v1/auth/google/callback", params={"code": "fake_code", "state": "U12345"})
