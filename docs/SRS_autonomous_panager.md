...
본 문서는 **자율 판단 AI 에이전트 "패니저(Panager)" v2.0**의 소프트웨어 요구사항을 정의합니다.
...
- **현재 버전**: v1.0 (Rule-based)
- **목표 버전**: v2.0 (Autonomous Agent)
...
| ID | 기능 | 제외 사유 |
|----|------|----------|
| OS-001 | 다중 사용자 지원 | v2.0에서는 단일 사용자만 지원 |
| OS-002 | 음성 인터페이스 | 텍스트 기반만 지원 |
| OS-003 | 금융 거래 | 조회만 가능, 실제 거래 불가 |
| OS-004 | 외부 캘린더 연동 | 자체 SQLite 캘린더만 사용 |
...
| 시스템 | 관계 | 설명 |
|--------|------|------|
| 패니저 v1.0 | 대체 | Rule-based → Autonomous로 전환 |
| Slack | 통합 유지 | 기존 Slack 연동 유지 |
| SQLite DB | 확장 | 기존 스키마에 학습 테이블 추가 |
...
| 외부 API | 무료 tier | OpenWeatherMap, Kakao Maps |

### 2.4.3 운영 제약

| ID | 제약사항 | 설명 |
|----|----------|------|
| OC-001 | 단일 사용자 | v2.0은 1명의 사용자만 지원 |
| OC-002 | 인터넷 필수 | Slack, LLM API 접근 필요 |
| OC-003 | 한국어 우선 | 한국어 대화 및 알림 |
...
| D-003 | OpenWeatherMap | 날씨 정보 | WeatherAPI, 기상청 API |
| D-004 | Kakao Maps API | 길찾기 | Naver Maps, Google Maps |
...
#### SI-003: OpenWeatherMap API

| 항목 | 내용 |
|------|------|
| **ID** | SI-003 |
| **명칭** | OpenWeatherMap Current Weather API |
| **용도** | 현재 날씨 조회 |
| **프로토콜** | HTTPS REST API |
...
| 정보 | 소스 | 주기 |
|------|------|------|
| 현재 시간 | System Clock | 매 루프 |
| 날씨 | OpenWeatherMap API | 30분 캐시 |
| 일정 | SQLite DB | 매 루프 |
| 최근 알림 | Memory | 매 루프 |
| 학습 교훈 | SQLite DB | 매 루프 |
