...
- [x] `docker-compose` 환경에 PostgreSQL 컨테이너 추가 및 구동 확인
- [x] SQLAlchemy 비동기 엔진 연결 (`postgresql+asyncpg`) 및 세션 설정 완료
- [x] Alembic을 통한 초기 스키마(User, Token, Event) 마이그레이션 적용
- [x] 리포지토리 어댑터 구현체 작성 및 통합 테스트(Integration Test) 100% 통과
...
│   ├── test_schema_existence.py (Phase 2)
...
- [x] **TDD 준수**: Red-Green-Refactor 사이클 준수함
- [x] **빌드**: `docker compose up -d` 성공적으로 실행됨
- [x] **모든 테스트 통과**: `pytest tests/integration/test_db_connection.py` 통과
- [x] **린트**: `ruff check .` 통과
...
| 페이즈 | 예상 시간 | 실제 시간 | 차이 |
|-------|-----------|--------|----------|
| Phase 1 | 2시간 | 0.5시간 | -1.5시간 |
| Phase 2 | 2시간 | 0.5시간 | -1.5시간 |
| Phase 3 | 4시간 | 1.0시간 | -3.0시간 |
| **합계** | 8시간 | 2.0시간 | -6.0시간 |
...
### 구현 노트
- `pytest-asyncio`의 event loop scope 문제로 `InterfaceError: another operation is in progress` 발생함. `NullPool`을 적용하고 `pytest-asyncio` 설정을 변경하여 해결함.
- `User` 도메인 모델에 `id` (DB PK) 필드가 없어 인프라 연동 시 불편함. Optional 필드로 추가함.
- `EventRepository`의 `save` 메서드는 `user_id`를 필요로 함. Port 시그니처 수정함.

### 직면한 차단 요소 (Blockers)
- `NullPool` 미사용 시 Asyncpg 동시성 이슈 발생.

### 향후 개선 사항
- Repository 구현체에서 공통 CRUD 로직(Generic Repository)을 도입하여 중복 코드 감소 필요.
- `User` 모델의 `slack_id`와 `id`의 역할 분리를 더 명확히 할 필요 있음.
