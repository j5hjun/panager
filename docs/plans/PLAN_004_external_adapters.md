...
**Estimated Completion**: 2026-01-09
...
- [ ] Google Calendar 이벤트 목록 조회 기능 구현 및 테스트 통과
- [ ] Google Calendar OAuth 토큰 갱신 로직 구현
- [ ] Slack 메시지 전송 기능 구현 및 테스트 통과
- [ ] 모든 어댑터가 도메인 Port 인터페이스를 구현
...
**🔴 RED: 실패하는 테스트 먼저 작성**
- [ ] **Test 1.1**: Port Import 테스트
  - 파일: `tests/unit/test_ports.py`
  - 상세: `CalendarPort`, `MessengerPort` 클래스 존재 여부 확인

**🟢 GREEN: 테스트 통과를 위한 구현**
- [ ] **Task 1.2**: CalendarPort 정의
  - 파일: `src/domain/ports/calendar_port.py`
  - 메서드: `get_events()`, `create_event()`, `watch_events()`
- [ ] **Task 1.3**: MessengerPort 정의
  - 파일: `src/domain/ports/messenger_port.py`
  - 메서드: `send_message()`, `send_block_message()`

#### 품질 게이트 (Quality Gate) ✋
- [ ] **TDD 준수**: Red-Green 사이클 준수
- [ ] **린트**: `ruff check .` 통과
...
- [ ] **Task 4.1**: Settings 업데이트
  - 파일: `src/config/settings.py`
  - 추가: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `SLACK_BOT_TOKEN`
- [ ] **Task 4.2**: 의존성 주입 설정
  - 파일: `src/infrastructure/container.py` (또는 FastAPI Depends)
  - 구현: Adapter 인스턴스 생성 및 주입
