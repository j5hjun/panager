...
### 3.5. 계획서 검토 및 승인 (Review)

**중요**: 계획서를 작성한 후 **즉시 구현을 시작하지 마십시오.**
반드시 사용자의 검토를 받고, 수정 사항이 있다면 반영한 뒤 구현 승인을 받아야 합니다.

- 계획서의 논리적 흐름 점검
- 테스트 전략 및 품질 게이트 적절성 확인
- **사용자 승인 후 구현 단계 진입**

## 4. 기능 계획서 기반으로 구현

승인된 계획서(`Status: In Progress`)를 바탕으로 구현을 시작합니다.
각 Phase가 끝날 때마다 Quality Gate를 통과해야 다음 Phase로 넘어갈 수 있습니다.

**중요**: 서버 실행 시 구현한 기능이 main.py에 연결되어 있는지 최종 확인합니다.

## 5. 최종 검증 및 계획서 완료 (Final Verification)
모든 페이즈가 완료되면, PR 생성 전 다음 사항을 검증하고 계획서를 완료 처리합니다.

1. **계획서 체크리스트 검증**:
   - `Success Criteria`, `Tasks`, `Quality Gate`의 모든 체크박스가 `[x]`로 체크되었는지 확인합니다.
   - 미체크 항목이 있다면 구현하거나 제외 사유를 기록해야 합니다.

2. **계획서 상태 변경**:
   - 검증이 끝났다면 파일 최상단 `Status`를 `✅ Complete`로 변경합니다.

3. **CI 검증**:
   - 로컬 환경에서 테스트 및 린트 검사를 통과해야 합니다.
